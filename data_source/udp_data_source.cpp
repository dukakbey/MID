...
void UDPDataSource::Init(DPUConfig* dpu_config)
{
  ...
  if (GetDPUConfig()->interrogator_fiber_parameters_.interrogator_usage_type == MIDASV2D)
  {
      input_priority_ = GetDPUConfig()->interrogator_fiber_parameters_.midas2d_fiber_parameter.input_priority;
  }
...
void UDPDataSource::HandleReceive(const boost::system::error_code& error, std::size_t bytes_transferred)
{
  ...
  else
  {
...
#if 0
      // temporary code to fix 14 byte overhead by NANOTAM
      ...
      // reset parameters and counters. reset_parameters_ default 1'dir. ilk 8 fragment alýndýðýnda 0'a çekiyor.
      ...
      else     // done with dynamic parameter initialization
      {
        ...
        //400 sample'a ulaþýldýðý zaman burdaki if'e giriyor. Burdan algoritma ve kayýt bloðuna yollanýyor.
        if (end_cnt_diff > 0 || end_cnt_diff < INT_MIN)
        {

          cout << "400 frame'e ulasildi.\nframe_count="<< ((int64_t)UDP_packet_header_->frame_count)<<"\n";
          ...
          if (free_buffer_chunk_.buffer_pointer != NULL)
          {
			  ...
			  {
			  case REDUNDANT:
			  case SEPERATE://ilk 1000 kanalý reverse etmek için
			  {
        ...
        // 400 frame'e ulaþmadýðý zaman yani 0-399 frame arasý bu yapý çalýþýr.
        if (start_cnt_diff >= 0 || start_cnt_diff < INT_MIN)
        {
            //int i = 0;
            //if(i++%10 == 0)cout << "0-399 frame arasi\n";
          // calculate hash table index
          // 400*5150'lik memory alaný var. bu fonksyion bu alana yazýlacak pozisyonu buluyor. Midas 3 seperate ise farklý bir hesaplama var.
        //idx ile her frame için fragment no tutuluyor  
        int64_t idx = ComputeHashTableIndex(UDP_packet_header_->frame_count);
          ...
          //cout << "wtite offset"<<write_offset << "\n";
